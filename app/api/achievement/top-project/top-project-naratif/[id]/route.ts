...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
    ...
    const { data: existing } = await supabase.from("achievement_top_project_naratif").select("id").eq("id", id).single();
    if (!existing) return NextResponse.json({ error: "Data tidak ditemukan.", code: "NOT_FOUND" }, { status: 404 });

    const { data, error } = await supabase
        .from("achievement_top_project_naratif")
        .update({
            title: body.title,
            detail: body.detail,
            urutan: urutan ?? 0,
    ...
    const { error: logError } = await supabase.from("activity_logs").insert({
        actor_id: user!.id, action: "update", entity_type: "achievement_top_project_naratif", entity_label: body.title,
    });
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
    ...
    const { data: existing } = await supabase.from("achievement_top_project_naratif").select("title").eq("id", id).single();
    if (!existing) return NextResponse.json({ error: "Data tidak ditemukan.", code: "NOT_FOUND" }, { status: 404 });

    const { error } = await supabase.from("achievement_top_project_naratif").delete().eq("id", id);
    if (error) return NextResponse.json({ error: error.message, code: "SERVER_ERROR" }, { status: 500 });

    const { error: logError } = await supabase.from("activity_logs").insert({
        actor_id: user!.id, action: "delete", entity_type: "achievement_top_project_naratif", entity_label: existing?.title ?? id,
    });
    if (logError) console.error("Gagal mencatat activity log:", logError.message);
