...
// GET: ambil semua data penghargaan HSSE
export async function GET() {
    try {
        ...
        return NextResponse.json(
            { error: err.message ?? "Gagal mengambil data penghargaan" },
            { status: 500 }
        );
    }
}

// POST: tambah data penghargaan baru
export async function POST(req: NextRequest) {
    try {
        ...
        console.error("Gagal menambah data penghargaan:", err);
        return NextResponse.json({ error: err.message ?? "Gagal menambah data penghargaan" }, { status: 500 });
    }
}