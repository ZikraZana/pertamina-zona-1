    ...
    useEffect(() => {
       ...
       function renderFacilityPage() {
        ...
        function selectProvinsi(provinsiCode: string) {
            if (activeProvinsi) {
                document.querySelector(`.prov[data-p="${activeProvinsi}"]`)?.classList.remove('fill-[#9fe400]');
            }
            activeProvinsi = provinsiCode;
            document.querySelector(`.prov[data-p="${activeProvinsi}"]`)?.classList.add('fill-[#9fe400]');
        }
        ...
        function handleGlobalClick(e: MouseEvent) {
            ...
            // Deteksi Klik Tombol Close (X) di Badge Panduan Pertama Kali
            if (target.closest('#closeFirstTimeHint')) {
                e.stopPropagation();
                hideFirstTimeHint();
                return;
            }
        ...
        <>
            <div className="flex min-h-screen flex-col overflow-visible bg-slate-100 p-3 sm:p-4 lg:h-screen lg:overflow-hidden lg:p-6">
                ...
                <div className="relative min-h-0 overflow-hidden rounded-xl border border-slate-300 bg-white p-3 shadow-lg sm:p-4 lg:flex-1">
                    <div className="pointer-events-none absolute left-1/2 top-3 z-20 w-[90%] max-w-xs -translate-x-1/2 px-2 sm:top-4 sm:w-auto sm:max-w-sm md:max-w-md">
                        <div
                            id="first-time-hint"
                            className="pointer-events-auto flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-[11px] font-semibold text-blue-900 shadow-lg transition-opacity duration-700 sm:px-4 sm:text-xs"
                            style={{ animation: 'hintBounce 1.8s ease-in-out infinite' }}
                        >
                            <span className="flex-1 leading-snug text-center sm:text-[10px] md:text-[11px]">Klik salah satu label untuk melihat detail wilayah kerja</span>
                            <button
                                type="button"
                                id="closeFirstTimeHint"
                                aria-label="Tutup panduan"
                                className="flex h-5 w-5 shrink-0 cursor-pointer items-center justify-center rounded-full bg-slate-50 text-slate-400 transition-colors hover:bg-red-50 hover:text-red-500"
                            >
